...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        distance2 = random.gauss(distance, result.distance_noise)
        
        beta = (distance2/result.length)* tan(steering2) # turning angle
    
        if(beta>0.001):
            r = distance2/beta # radius
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for x in range(len(landmarks)):
            bearing = atan2(landmarks[x][0]-self.y, landmarks[x][1] - self.x) - self.orientation
            if(add_noise):
